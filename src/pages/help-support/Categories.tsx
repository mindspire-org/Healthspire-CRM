...
export default function HelpSupportCategories() {
  ...
  const fetchAll = async () => {
    setLoading(true);
    try {
      const r = await getJson(`/api/help/categories?scope=help`);
  ...
  const save = async () => {
    ...
    if (!payload.name || !payload.name.trim()) { setError("Name is required"); return; }
    const method = editing?"PUT":"POST";
    const res = await sendJson(`/api/help/categories${editing?`/${editing._id}`:""}`, method, payload);
    if (!res.ok) {
  ...
  const remove = async (id?: string) => {
    if (!id) return;
    const res = await sendJson(`/api/help/categories/${id}`, "DELETE");
  ...
  return (
    <div className="space-y-4 animate-fade-in">
      <div className="flex items-center justify-between">
        <h1 className="text-base font-semibold">Help - Categories</h1>
