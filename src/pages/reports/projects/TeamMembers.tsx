...
export default function ProjectsTeamMembers() {
  ...
  const teamAgg = useMemo(() => {
    const map = new Map<string, { name: string; open: number; completed: number; hold: number; openTasks: number; completedTasks: number; totalTime: number; totalHours: number }>();
    for (const p of projects) {
      const status = p?.status as string | undefined;
      const arr: string[] = Array.isArray(p?.members) ? p.members : [];
      const inc = (row: any) => {
        if (isCompleted(status)) row.completed += 1; else if (isHold(status)) row.hold += 1; else row.open += 1;
      };
      if (!arr.length) continue;
      for (const m of arr) {
        const name = String(m || "-");
        const row = map.get(name) || { name, open: 0, completed: 0, hold: 0, openTasks: 0, completedTasks: 0, totalTime: 0, totalHours: 0 };
        inc(row);
        map.set(name, row);
      }
    }
    const q = norm(query);
    return Array.from(map.values()).filter(r => !q || norm(r.name).includes(q)).sort((a,b)=> (b.open + b.completed + b.hold) - (a.open + a.completed + a.hold));
  }, [projects, query]);

  // Aggregation: by client
  const clientsAgg = useMemo(() => {
    const map = new Map<string, { client: string; open: number; completed: number; hold: number; openTasks: number; completedTasks: number; totalTime: number; totalHours: number }>();
    for (const p of projects) {
      const status = p?.status as string | undefined;
      const key = String(p?.client || "-");
      const row = map.get(key) || { client: key, open: 0, completed: 0, hold: 0, openTasks: 0, completedTasks: 0, totalTime: 0, totalHours: 0 };
      if (isCompleted(status)) row.completed += 1; else if (isHold(status)) row.hold += 1; else row.open += 1;
      map.set(key, row);
    }
    const q = norm(query);
    return Array.from(map.values()).filter(r => !q || norm(r.client).includes(q)).sort((a,b)=> (b.open + b.completed + b.hold) - (a.open + a.completed + a.hold));
  }, [projects, query]);
